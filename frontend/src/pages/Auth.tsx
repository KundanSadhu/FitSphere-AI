...
export function AuthPage({ onSuccess }: AuthPageProps) {
  ...
  const handleEmailAuth = async (e: FormEvent) => {
    ...
    try {
      ...
      else if (err.code === 'auth/invalid-login-credentials') errorMsg = 'Invalid email or password.';
      ...
      else if (err.code === 'auth/operation-not-allowed') errorMsg = 'Authentication provider is disabled. Enable Email/Password in Firebase platform.';
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      if (err.code === 'auth/operation-not-allowed') {
        setError('Google Sign-In is not enabled. Please go to your Firebase Console > Authentication > Sign-in method and enable the Google provider.');
      } else if (err.code === 'auth/popup-closed-by-user' || err.code === 'auth/cancelled-popup-request') {
        setError(null); // Ignore it if user simply cancelled the popup
      } else {
      ...
      <div className="w-full max-w-md bg-white border-2 border-[#191A23] rounded-[24px] shadow-[6px_6px_0px_#191A23] p-8">
        ...
        <div className="relative mb-6">
          ...
          <div className="relative flex justify-center text-xs">
            <span className="px-3 bg-white text-slate-500 font-extrabold uppercase">Or continue with</span>
          </div>
        </div>
